...
| DE on design v2 (2026-09-26): new /de/ home + Shopify development + 5 service pages on the v2 templates (same paths as EN/UK), German 404 in the v2 shell; hreflang en/de/uk + x-default=EN on EN/UK/DE; menus, footer and related links stay in /de/ (localHref); EUR budget bands in German with "Bis 5.000 €" off Complex; sitemap + llms.txt "Deutsch" | REQ-019 | review | copy = content/pages/<page>.de.json from the copywriter (parallel branch); until it lands the DE pages render the EN copy with a German fallback title/description in the wrapper. No German terms page (would duplicate the EN text): DE footer keeps "Terms (EN)". qa/tests/de.spec.js; German-copy checks run once home.de.json exists. de/404.html is built but not an ErrorDocument (same as uk/404.html) |
