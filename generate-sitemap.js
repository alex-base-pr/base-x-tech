...
// Absorbed/noindex pages (EN and UK) stay live but are left out. Home pages ('/', '/de/', '/uk/') get priority 1.0.
const pages = siteMap.pages
  .filter((p) => p.role === 'index')
  .map((p) => ({ url: p.path, priority: p.group === 'home' ? 1.0 : 0.8 }));

// Guard: every UK and DE page on disk must be declared in the site map (else it would silently fall out of the sitemap).
...
const undeclared = ['uk', 'de'].flatMap((d) => walk(path.join(__dirname, d))).map((f) => path.relative(__dirname, f).replace(/\\/g, '/')).filter((f) => !declared.has(f));
if (undeclared.length) console.warn(`sitemap: UK/DE pages missing from json/site-map.json: ${undeclared.join(', ')}`);
