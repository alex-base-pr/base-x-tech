...
import { siteMap, host, indexPages, servicePaths, ukIndexPages, deIndexPages } from '../site-map.js';
...
test('T-005 sitemap.xml = indexable EN + DE + UK sets, canonical form', async () => {
  ...
  const de = locs.filter((u) => u.startsWith(host + '/de/')).sort();
  expect(de, 'DE = the indexable DE pages of the site map').toEqual(deIndexPages.map((p) => host + p.path).sort());
});
