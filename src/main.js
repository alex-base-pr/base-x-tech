...
class DrawerManager {
  ...
  initListeners() {
    document.addEventListener("click", (event) => {
      for (const [key, { openTrigger, closeTrigger }] of Object.entries(
        this.drawers,
      )) {
...
document.body.addEventListener("click", (event) => {
  if (document.body.classList.contains("block-scroll")) {
    if (
      ...
      !event.target.closest(".drawer-form--wrapper")
    ) {
...
document.addEventListener(
  "DOMContentLoaded",
  () => new DrawerManager(drawerConfig),
);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function checkTopPosition() {
    if (
      window.scrollY <= offsetThreshold &&
      header.classList.contains("in-scroll")
    ) {
      header.classList.remove("in-scroll");
      scrollUpDistance = 0;
    }
...
(function () {
  const langSwitcherLinks = document.querySelectorAll(
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var menuLinks = document.querySelectorAll(
    ".desk-nav-menu a, .drawer-nav--wrapper a",
  );
